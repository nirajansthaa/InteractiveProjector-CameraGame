...
def save_calibration_points(points, offset_x=0, offset_y=0, filename=CALIBRATION_FILE):
    try:
        calibration_data = {
            'points': points,
            'offset_x': offset_x,
        ...
        print(f"Calibration saved to {filename} with offset ({offset_x}, {offset_y})")
    except Exception as e:
...
def load_calibration_points(filename=CALIBRATION_FILE):
    try:
        ...
        if isinstance(data, dict) and 'points' in data:
            ...
            return data['points'], offset_x, offset_y
        return None, 0, 0
    except Exception as e:
...
calibration_points, offset_x, offset_y = load_calibration_points()
transform_matrix = None
if calibration_points and len(calibration_points) == 4:
    print(f"Loading existing calibration with offset ({offset_x}, {offset_y})...")
    ...
    if calibration_points and len(calibration_points) == 4:
        save_calibration_points(calibration_points, offset_x, offset_y)
        transform_matrix = get_perspective_transform(calibration_points, offset_x, offset_y)
        test_calibration_accuracy(transform_matrix, calibration_points)
    else:
...
while running:
    ...
    cv2.putText(debug_view, f"Screen: {SCREEN_WIDTH}x{SCREEN_HEIGHT}", (10, 30),
                cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
    cv2.putText(debug_view, f"Offset: ({offset_x}, {offset_y})", (10, 60),
                cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if event.key == pygame.K_q:
                ...
                calibration_points = get_calibration_points(cap)
                if calibration_points and len(calibration_points) == 4:
                    offset_x, offset_y = 0, 0  # Reset offsets
                    save_calibration_points(calibration_points, offset_x, offset_y)
                    transform_matrix = get_perspective_transform(calibration_points, offset_x, offset_y)
                    test_calibration_accuracy(transform_matrix, calibration_points)
    ...
    if show_debug_overlay:
        ...
        for i, pt in enumerate(dst_points):
            pygame.draw.circle(screen, (0, 255, 255), (int(pt[0]), int(pt[1])), 10)
            pygame.draw.circle(screen, (255, 255, 255), (int(pt[0]), int(pt[1])), 10, 2)
    offset_text = font.render(f"Offset: ({offset_x}, {offset_y})", True, (255, 255, 255))
